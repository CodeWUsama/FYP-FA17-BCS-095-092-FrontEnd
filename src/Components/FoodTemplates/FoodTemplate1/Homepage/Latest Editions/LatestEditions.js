...
export default class LatestEditions extends Component {
    ...
    render() {
        ...
        let displayDishes = array.map((dish, index) => {
            if (count < 3) {
                count++;
                return <Dish flexBasis="33%" dishId={dish._id} key={dish._id} backgroundImg={"url('data:image/jpeg;base64," + this.state.imgs[index]} dishName={dish.name} desc={dish.desc} />
            }
        });
