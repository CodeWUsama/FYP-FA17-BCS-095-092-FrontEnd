...
const ContactRequest = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            await fetch("http://localhost:8080/s1td/viewMessages?id=" + localStorage.getItem("id"), {
                method: "GET",
                headers: {
    ...
    let handleReply = () => {
        fetch("http://localhost:8080/s1td/reply", {
            method: "POST",
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token'),
                "Content-Type": 'application/json'
    ...
    let handleDelete = (id) => {
        fetch("http://localhost:8080/s1td/deleteMessage", {
            method: "POST",
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token'),
                "Content-Type": 'application/json'
