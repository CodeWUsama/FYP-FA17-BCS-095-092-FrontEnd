...
import SideEditor from "./../Reusable Components/SideEditor/SideEditor";
import TextEditor from "./../Reusable Components/Editors/TextEditor/TextEditor";
...
export default class LandingPage extends Component {
    ...
    saveChangesHandler = () => {
        fetch("http://localhost:8080/s1td/updateData", {
            method: "POST",
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token'),
                "Content-Type": 'application/json'
    ...
    changeSectionDesc = (newName, newSize, newFamily, newColor, bold, italic, underline, align) => {
        if (newSize === '') {
            newSize = 0;
        }
        this.setState({
    ...
    changeSectionName = (newName, newSize, newFamily, newColor, bold, italic, underline, align) => {
        if (newSize === '') {
            newSize = 0;
        }
        this.setState({
    ...
    render() {
        ...
        if (this.state.sectionNameEditor === true) {
            textEditor = <TextEditor closeHandler={() => { this.setState({ sectionNameEditor: false }) }} changeHandler={(newNam, newSiz, newFamil, newColo, bold, italic, underline, align) => this.changeSectionName(newNam, newSiz, newFamil, newColo, bold, italic, underline, align)} data={this.state.sectionName} />
        }

        if (this.state.sectionDescEditor === true) {
            textEditor = <TextEditor closeHandler={() => { this.setState({ sectionDescEditor: false }) }} changeHandler={(newNam, newSiz, newFamil, newColo, bold, italic, underline, align) => this.changeSectionDesc(newNam, newSiz, newFamil, newColo, bold, italic, underline, align)} data={this.state.sectionDesc} />
        }
            ...
            <div className={classes.rootCont}>
                ...
                <div className={classes.content} style={this.state.showEditor ? { width: "65%" } : { width: "100%" }}>
                    ...
                    <AboutUs
                        ...
                        sectionName={this.state.sectionName}
                        sectionNameHandler={this.sectionNameEditor}

                        sectionDesc={this.state.sectionDesc}
                        sectionDescHandler={this.sectionDescEditor}
