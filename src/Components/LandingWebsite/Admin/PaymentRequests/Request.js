...
const Request = (props) => {
    ...
    let handleApprove = () => {
        fetch("http://localhost:8080/admin/approveRequest", {
            method: "POST",
            headers: {
    ...
    let handleDiscard = () => {
        fetch("http://localhost:8080/admin/discardRequest", {
            method: "POST",
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token'),
                "Content-Type": 'application/json'
    ...
    return (
        <div className={classes.requestCont}>
            ...
            <p className={classes.text}><strong>Package: </strong>{props.amount}</p>
