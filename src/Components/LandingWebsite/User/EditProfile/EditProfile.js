...
export default class EditProfile extends Component {
    ...
    async componentDidMount() {
        await fetch("http://localhost:8080/user/getUserData", {
            headers: {
    ...
    handleSubmitPass = (event) => {
        event.preventDefault();
        fetch("http://localhost:8080/user/updatePass", {
            method: "POST",
            headers: {
                Authorization: "Bearer " + localStorage.getItem('token'),
                "Content-Type": 'application/json'
            ...
            <div>
                <div className={classes.info}>
                    <form className={classes.inpu} onSubmit={this.handleSubmitProfile}>
                        ...
                        <TextField value={this.state.fullname} className={classes.inputField} error={this.state.validFullname} helperText={this.state.validFullname ? "Only Alphabets are allowed!" : ""} onChange={this.validateName} id="fullname" type="text" label="Full Name" required={true} />
                        ...
                        <div className={classes.btn}>
                            <button type="submit" style={{ width: '40%' }} className="btn btn-primary btn-block btn-large" >Update</button>
                        </div>
                    </form>
                    <form className={classes.inpu} onSubmit={this.handleSubmitPass}>
                        ...
                        <div className={classes.btn}>
                            <button disabled type="submit" id="submitBtn" style={{ width: '40%' }} className="btn btn-primary btn-block btn-large" >Update</button>
                        </div>
                    </form>
                </div>
                <Footer />
