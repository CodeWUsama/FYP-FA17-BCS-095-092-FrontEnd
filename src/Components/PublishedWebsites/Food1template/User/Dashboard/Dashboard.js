...
export default class Dashboard extends Component {
    ...
    async componentDidMount() {
        ...
        await fetch("http://localhost:8080/f1td/viewOrders", {
            method: "POST",
            headers: {
    ...
    render() {
        ...
        if (this.state.orders) {
            displayData = this.state.orders.map(order => {
                console.log(order);
                return (
                    <tr>
        ...
        return (
            <div className={classes.rootCont}>
                ...
                <div className={classes.btCont}>
                    <button onClick={() => { window.location.href = "/RestaurantWebsite/#/editProfile" }} className={classes.btt}>Edit Profile</button>
